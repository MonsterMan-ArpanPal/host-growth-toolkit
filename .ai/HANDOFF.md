...
- Pricing model compatibility: the saved V1.2 model requires scikit-learn 1.7.2. Both workspace manifests are pinned, the rebuilt `.venv` now uses compatible NumPy/Pandas/SciPy and makes base-model predictions successfully. Full recommendations still require the currently missing `data/raw/listings.csv.gz` comparable-listings dataset.
...
- 2026-09-19: New signups now open the New Listing flow instead of legacy Property Setup. Existing users with either a stored property or an owned TinyDB listing land on Overview; saving a listing also records it as the account's active property for future sign-ins. Auth checks and frontend build pass.
- 2026-09-19: Pricing page now shows a dedicated, styled empty state for hosts with no properties, explaining how recommendations become available and linking directly to Create New Listing. Frontend build passes.
- 2026-09-19: Hardened TinyDB listing storage: empty or unreadable JSON is backed up when possible and recreated as a valid empty store, with each listing operation retrying after recovery. A successful empty account-property response now stays empty instead of falling through to mock properties, preventing pricing 404s for new hosts. API checks and frontend build pass.
- 2026-09-19: Simplified the Pricing empty state: removed the “Dynamic pricing” eyebrow and CTA icon, and tightened spacing between its explanatory copy and action. Frontend build passes.
- 2026-09-19: Removed the sticky pricing-API-unavailable flag. A temporary connection failure now uses fallback data only for that request; every later pricing/calendar request retries the live backend automatically. Frontend build passes.
- 2026-09-19: Replaced destructive TinyDB recovery with process-locked atomic JSON storage. Listing writes now complete to a temporary file and atomically replace the live store; unreadable storage returns 503 without changing data. A concurrent 20-write test retained all records and valid JSON.
- 2026-09-19: A missing or truly zero-byte `listings.json` now initializes safely as `{"_default": {}}`; non-empty malformed data remains untouched and returns 503. Temporary-store verification passed.
- 2026-09-19: Listing saves may encounter temporary Windows/OneDrive file locks during atomic replacement. The storage writer now retries replacement five times with short backoff before returning 503, and logs the exact persistent storage exception. Temporary-store write verification passed.
- 2026-09-19: Moved mutable TinyDB listings and uploaded photos outside the OneDrive checkout to `%LOCALAPPDATA%\\Wayzyy` (overridable via `WAYZYY_DATA_DIR`), eliminating OneDrive replacement locks. New local store initialization and listings API check pass.
- 2026-09-19: Added frontend and backend validation for pricing dates. Invalid or impossible `YYYY-MM-DD` values are blocked before requests and rejected with a clear API validation message as defense in depth. API checks and frontend build pass.
